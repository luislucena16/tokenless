...
import { DerivedProtocolTotal, DerivedProject } from "./common-types.js";
import { Project, QualifiedCoinInfo } from "@tokenless/tokenless-contract";
...
export function createDerivedProjectsArray(projects: {
    isEmpty(): boolean;
    size(): bigint;
    member(key_0: Uint8Array): boolean;
    lookup(key_0: Uint8Array): Project;
    [Symbol.iterator](): Iterator<[Uint8Array, Project]>
}): DerivedProject[] {
  return Array.from(projects).map(([key, project]) => ({
    id: uint8arraytostring(key),
    project: project,
  }));
}
...
export default {randomNonceBytes, uint8arraytostring, createDeriveProtocolTVLArray, createDerivedProjectsArray, pad  };