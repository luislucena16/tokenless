...
import { TokenlessAPI, type DeployedTokenlessAPI } from "@tokenless/tokenless-api";
...
export interface DeploymentProvider {
  ...
  readonly tokenlessApi: DeployedTokenlessAPI | undefined;
...
const DeployedContractProvider = ({ 
  ...
  const [tokenlessApi, setTokenlessApi] = useState<DeployedTokenlessAPI | undefined>(undefined);
  ...
  const onJoinContract = useCallback(async (address: string) => {
    ...
    try {
      const deployedAPI = await TokenlessAPI.joinTokenlessContract(
        walletContext.walletState.providers,
        address,
        logger
      );
      
      setTokenlessApi(deployedAPI);
      setHasJoined(true);
  ...
  const onDeployContract = useCallback(async () => {
    ...
    try {
      const deployedAPI = await TokenlessAPI.deployTokenlessContract(
        walletContext.walletState.providers,
        logger
      );
