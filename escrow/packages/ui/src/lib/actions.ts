...
import {
  EscrowAPI,
  type EscrowContractProviders,
  type DeployedEscrowAPI,
  type EscrowCircuitKeys,
} from "@escrow/escrow-api";
...
export const initialWalletAndProviders =
  async (): Promise<WalletAndProvider> => {
    ...
    const providers = {
      ...
      zkConfigProvider: new FetchZkConfigProvider<EscrowCircuitKeys>(
...
): Promise<DeployedEscrowAPI> => {
  let api;
  if (contractAddress) {
    api = await EscrowAPI.joinEscrowContract(
      providers,
      contractAddress,
      logger
    );
  } else {
    api = await EscrowAPI.deployEscrowContract(providers, logger);
  }
